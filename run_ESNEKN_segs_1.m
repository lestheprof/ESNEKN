function run_ESNEKN_segs_1(indatafile, trainingfraction, segdir, re_size, leakage_rate, win_scl, w_scl, ...
    varargin)
...
test_segment_size = zeros([1 testdatalength * 50]) ;
for (testindex = traindatalength + 1:datalength)
    ...
    for segno=1:size(currentsegs.segments,1)
        ...
        test_segment_size(testdataindex) = currentsegs.segments(segno,2) - currentsegs.segments(segno,1) ;
        testdataindex = testdataindex + 1 ;
    end % for
end % for
testing_Data = testing_Data(1:testdataindex - 1) ;
testing_label = testing_label(1:testdataindex - 1) ;
test_segment_size = test_segment_size(1:testdataindex - 1) ;
                ...
                for cos=1 : length (c_sent)
                    for g=1 : length (g_sent)
                        for b=1 : numberOfRuns
                            ...
                            % new version returns TY the training output, n_classes by # of test data
                            % elements
                            [ accuracyTraining_ESNEKMs, accuracyTesting_ESNEKMs, TY, conf_matrix] = ...
                                elm_kernel_1('train_Data_extreme.csv', 'test_Data_extreme.csv', 1, c_sent(cos), 'RBF_kernel',g_sent(g));
                            % calculate new value for accuracyTesting_ESNEKMs using TY,  testing_label, testing_Data and  testing_soundfilename
                            % use the segment-size weighted averages of the
                            % classes found.
                            testindex = 1 ; % indexes the files
                            testsegindex = 1 ; % indexes the segments
                            numclasses = size(TY, 1) ;
                            conf_matrix_seg = zeros(numclasses) ;
                            misclassified = 0 ;
                            while testindex <= testdatalength
                                filename = testing_soundfilename(testsegindex) ;
                                segclasses = zeros([1 numclasses]) ; % array to hold clasifications found for this file
                                % create array of classification indexes
                                while ((testsegindex <= length(testing_soundfilename)) &&  (strcmp(testing_soundfilename{testsegindex}, cell2mat(filename))))
                                    [~, label_index_actual]=max(TY(:,testsegindex));
                                    segclasses(label_index_actual) = segclasses(label_index_actual)  + ...
                                        (test_segment_size(testsegindex) * 1);
                                    testsegindex = testsegindex + 1 ;
                                end % while
                                [~, classfound] = max(segclasses) ;
                                conf_matrix_seg(testing_label(testsegindex-1), classfound) =  conf_matrix_seg(testing_label(testsegindex-1), classfound) + 1;
                                if (testing_label(testsegindex-1) ~= classfound)
                                    misclassified = misclassified + 1 ;
                                end
                                testindex = testindex + 1 ;
                            end % while
                            accuracyTesting_ESNEKMs = (testdatalength - misclassified)/testdatalength ;
                            ...
                            disp([ '==================== Run : ' num2str(b) ' / '  num2str(numberOfRuns) '    Ends ===========================' newline]);
